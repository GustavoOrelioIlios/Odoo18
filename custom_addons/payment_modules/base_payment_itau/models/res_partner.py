...
class ResPartner(models.Model):
    ...
    itau_interest_code = fields.Selection([
       ...
       help='Código do tipo de juros aplicado após o vencimento para boletos Itaú'
    )
    
    itau_interest_percent = fields.Float(
        string='Percentual de Juros (Itaú)',
        digits='Account',
        help='Percentual de juros aplicado para boletos Itaú (quando aplicável)'
    )
    
    itau_interest_value = fields.Float(
        string='Valor Fixo de Juros (Itaú)',
        digits='Account',
        help='Valor fixo de juros aplicado para boletos Itaú (quando aplicável)'
    )
    
    itau_interest_date_start = fields.Integer(
        string='Dias para Início dos Juros (Itaú)',
        default=1,
        help='Número de dias após o vencimento para iniciar a cobrança de juros no Itaú'
    )
    ...
    itau_penalty_code = fields.Selection([
       ...
       help='Código do tipo de multa aplicado após o vencimento para boletos Itaú'
    )
    
    itau_penalty_percent = fields.Float(
        string='Percentual de Multa (Itaú)',
        digits='Account',
        help='Percentual de multa aplicado para boletos Itaú (quando aplicável)'
    )
    
    itau_penalty_value = fields.Float(
        string='Valor Fixo de Multa (Itaú)',
        digits='Account',
        help='Valor fixo de multa aplicado para boletos Itaú (quando aplicável)'
    )
    
    itau_penalty_date_start = fields.Integer(
        string='Dias para Início da Multa (Itaú)',
        default=1,
        help='Número de dias após o vencimento para iniciar a cobrança da multa no Itaú'
    ) 